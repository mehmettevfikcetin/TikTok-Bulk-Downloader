...
class ErrorDialog(QDialog):
    def __init__(self, error_list, parent=None):
        ...
        info_label = QLabel(f"Toplam {len(error_list)} video indirilemedi. Detaylar:")
        ...
        for i, item in enumerate(error_list):
            ...
            self.table.setItem(i, 1, QTableWidgetItem(item['error']))
            
        layout.addWidget(self.table)
        
        btn_layout = QHBoxLayout()
        copy_btn = QPushButton("URL'leri Kopyala")
        ...
        # Stil (Ana tema ile uyumlu)
        self.setStyleSheet("""
            QDialog { background-color: #252535; color: white; }
            QLabel { color: white; }
            QTableWidget { background-color: #303045; color: white; gridline-color: #555; border: 1px solid #555; }
            QHeaderView::section { background-color: #404060; color: white; padding: 5px; }
            QPushButton { background-color: #00bfff; color: white; border-radius: 5px; padding: 8px; font-weight: bold; }
    ...
    def copy_all_urls(self):
        ...
        QMessageBox.information(self, "Kopyalandı", "Hatalı linkler panoya kopyalandı.")
...
class LinkFetcherThread(QThread):
    ...
    def run(self):
        try:
            ...
            # --- GÜÇLENDİRİLMİŞ KOMUT (MOBİL MODU + CACHE TEMİZLİĞİ) ---
            command = [
                YT_DLP_PATH,
                ...
                '--no-cache-dir',  # ÖNEMLİ: Eski önbelleği kullanma, taze veri çek
                '--retries', '10', # Hata alırsan 10 kere tekrar dene
                '--fragment-retries', '10',
                '--sleep-requests', '2', # Her istek arası 2 saniye bekle (Biraz daha yavaş ama kararlı)
                # Kendini bir iPhone (Safari) gibi göster. TikTok mobilde daha cömerttir.
...
class DownloaderThread(QThread):
    ...
    # GÜNCELLEME 1: Sinyale 'list' parametresi eklendi (Hata listesi için)
    ...
    def __init__(self, urls, target_dir):
        ...
        # GÜNCELLEME 2: Hatalı videoları tutacak boş liste oluşturuldu
        self.failed_list = []
    ...
    def get_existing_video_ids(self):
        """Klasördeki dosyaları tarayıp mevcut video ID'lerini bir set olarak döndürür."""
        existing_ids = set()
        ...
        except Exception as e: print(f"DEBUG: Klasör tarama hatası: {e}")
        return existing_ids

    def extract_id_from_url(self, url):
        """URL içinden Video ID'sini regex ile çeker."""
        match = re.search(r'/video/(\d+)', url)
        if match: return match.group(1)
        return None

    def run(self):
        print("DEBUG: Klasördeki mevcut videolar taranıyor...")
        existing_ids_in_folder = self.get_existing_video_ids()
        print(f"DEBUG: {len(existing_ids_in_folder)} adet mevcut video tespit edildi.")
        ...
        for i, url in enumerate(self.urls):
            ...
            # 1. HIZLI KONTROL (Senin kodundaki özellik)
            quick_id = self.extract_id_from_url(url)
            if quick_id and quick_id in existing_ids_in_folder:
                status_message = "ATLANDI (Mevcut)"
                self.stats['skipped'] += 1
                self.videoFinished.emit(i, quick_id, status_message)
            ...
            try:
                ...
                if os.path.exists(output_path):
                    status_message = "ATLANDI (Dosya Var)"; self.stats['skipped'] += 1
                ...
                if not os.path.exists(output_path):
                    status_message = f"ATLANDI (yt-dlp?)"; self.stats['skipped'] += 1
                    self.videoFinished.emit(i, video_id, status_message)
                    if self.running: time.sleep(0.5); continue
                ...
                if not self.running: break
                 ...
                 # GÜNCELLEME 3: Hata detaylarını yakalayıp listeye ekliyoruz
                 err_msg = str(e)
                 if isinstance(e, yt_dlp.utils.DownloadError): status_message = f"HATA (yt-dlp: {err_msg[:80]}...)"
                 else: status_message = f"HATA (Genel: {err_msg[:80]}...)"
                 
                 self.stats['errors'] += 1
                 # Listeye ekle:
                 self.failed_list.append({'url': url, 'error': err_msg})
                 
                 self.videoFinished.emit(i, video_id, status_message)
                 if self.running: time.sleep(1)

        # GÜNCELLEME 4: Bitiş sinyali ile listeyi de gönderiyoruz
...
class App(QWidget):
    ...
    def initUI(self):
        ...
        self.status_table.setSelectionBehavior(QTableWidget.SelectionBehavior.SelectItems) # Sadece hücreleri seç
        self.status_table.setSelectionMode(QAbstractItemView.SelectionMode.ContiguousSelection) # Birden fazla hücre seçebilme
    ...
    def on_all_finished(self, completed, skipped, errors, failed_list): # Parametreye failed_list eklendi
        ...
        # Stil
        msgBox.setStyleSheet("QMessageBox { background-color: #252535; font-size: 11px; } QLabel { color: #e0e0e5; font-size: 11px; } QPushButton { background-color: #353555; color: white; padding: 5px; border: 1px solid #555; }")
        
        try:
            msgBox.setIconPixmap(QPixmap(resource_path("icon.ico")).scaled(64, 64, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
        except: 
        ...
        if errors > 0:
            ...
            show_errors_btn.setStyleSheet("background-color: #e74c3c; color: white; font-weight: bold;")
        
        ok_btn = msgBox.addButton("Tamam", QMessageBox.ButtonRole.AcceptRole)
        ok_btn.setStyleSheet("background-color: #00bfff; color: white; font-weight: bold;")
        
        msgBox.exec()

        # Butona basıldıysa detay penceresini aç
